...
const mainUseCases = [
  ...
  },
  {
    ...
    image: "https://cdn2.fptshop.com.vn/unsafe/Uploads/images/tin-tuc/173610/Originals/cong-nghe-so-la-gi-1.jpg",
    imageBadge: "Từ tài liệu rời rạc đến nguồn thông tin có thể sử dụng"
  },
  {
    id: "03",
      ...
      <div className="mx-auto max-w-360 px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col gap-6 lg:gap-8">
          ...
          {mainUseCases.map((item) => (
            <div key={item.id} className={`${item.bg} rounded-3xl lg:rounded-[2.5rem] p-6 lg:p-12 ${item.border}`}>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
                
                {/* Content Block */}
                <div className={`flex flex-col order-2 ${item.layout === 'right-text' ? 'lg:order-2' : 'lg:order-1'}`}>
                  <span className="text-[11px] font-semibold uppercase tracking-wider text-blue-600 mb-3 block">
                    {item.badge}
                  </span>
                  <h3 className={`text-[1.5rem] font-semibold leading-tight tracking-tight text-[#0a1a2f] sm:text-3xl lg:text-[2rem] mb-4 lg:mb-6 ${item.layout === 'custom' ? 'pr-4' : ''}`}>
                    {item.title}
                  </h3>
                ...
                {item.layout === 'custom' ? (
                  ...
                  <div className={`relative w-full ${item.id === '01' ? 'aspect-16/10 lg:aspect-4/3' : 'aspect-16/10'} rounded-2xl overflow-hidden ${item.id === '01' ? 'shadow-lg border border-slate-200/50' : 'bg-slate-100'} order-1 ${item.layout === 'right-text' ? 'lg:order-1' : 'lg:order-2'}`}>
                    {item.image && (
                      <Image src={item.image} alt={item.title} fill className="object-cover" unoptimized />
                    )}
                    {item.imageBadge && (
                      <div className="absolute top-4 right-4 bg-white/95 backdrop-blur px-4 py-2.5 rounded-xl shadow-lg border border-slate-100 flex items-center gap-2">
                        <span className="text-[12px] font-semibold text-slate-800 hidden sm:block">{item.imageBadge}</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            {splitUseCases.map((item) => (
              <div key={item.id} className={`${item.bg} rounded-3xl lg:rounded-[2.5rem] p-6 lg:p-12 ${item.border} flex flex-col`}>
                ...
                <div className="order-2 flex flex-col flex-1">
                  ...
                  <Link href="/lien-he" className={`inline-flex items-center gap-2 text-blue-600 font-semibold text-[14.5px] hover:text-blue-800 transition-colors w-fit ${item.id === '06' ? 'lg:mt-auto lg:mb-0' : 'lg:mb-8'}`}>
                    Trao đổi chi tiết &rarr;
                  </Link>
                </div>
