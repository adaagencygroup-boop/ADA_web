...
"ADA Group xây dựng hệ thống giúp doanh nghiệp tập trung thông tin khách hàng và kết nối với hoạt động bán hàng, chăm sóc và quản lý, tạo góc nhìn rõ ràng về khách hàng và nâng cao hiệu quả phối hợp giữa các bộ phận.",
      imagePlaceholder:"/images/dich-vu/he-thong/EnterpriseSolutions1.webp",
      icon: (
...
"ADA Group phát triển hệ thống giúp doanh nghiệp tổ chức thông tin nhân sự và các hoạt động nội bộ trên một môi trường thống nhất, giúp nhân viên, quản lý và các bộ phận phối hợp thuận tiện hơn, đồng thời tạo nền tảng hỗ trợ doanh nghiệp quản lý và phát triển đội ngũ một cách lâu dài.",
      imagePlaceholder:"/images/dich-vu/he-thong/EnterpriseSolutions2.webp",
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
...
"ADA Group xây dựng các hệ thống giúp doanh nghiệp tập trung hoạt động, dữ liệu và thông tin vận hành trên một môi trường thống nhất, giúp các bộ phận dễ dàng theo dõi, phối hợp và quản lý công việc hiệu quả hơn. Giải pháp được thiết kế linh hoạt theo đặc thù từng doanh nghiệp và có khả năng mở rộng khi quy mô, nhu cầu vận hành thay đổi.",
      imagePlaceholder:"/images/dich-vu/he-thong/EnterpriseSolutions3.webp",
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
...
"ADA Group xây dựng các hệ thống giúp doanh nghiệp tập trung hoạt động, dữ liệu và thông tin vận hành, giúp các bộ phận dễ dàng theo dõi, phối hợp và quản lý công việc. Giải pháp được thiết kế linh hoạt và có thể mở rộng theo nhu cầu phát triển của doanh nghiệp.",
      imagePlaceholder:"/images/dich-vu/he-thong/EnterpriseSolutions4.webp",
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
...
"ADA Group xây dựng các nền tảng giúp doanh nghiệp tổng hợp, quan sát và khai thác dữ liệu một cách trực quan, giúp thông tin được trình bày rõ ràng theo từng nhu cầu, từ theo dõi hoạt động hằng ngày đến hỗ trợ quản lý có góc nhìn tổng quan.",
      imagePlaceholder:"/images/dich-vu/he-thong/EnterpriseSolutions5.webp",
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
...
"ADA Group phát triển giải pháp có khả năng kết nối với website, ứng dụng, phần mềm và các nguồn dữ liệu hiện có, giúp thông tin được trao đổi xuyên suốt và hạn chế dữ liệu phân tán. Qua đó, doanh nghiệp có thể tận dụng hạ tầng công nghệ hiện tại và từng bước xây dựng môi trường vận hành thống nhất hơn.",
      imagePlaceholder:"/images/dich-vu/he-thong/EnterpriseSolutions6.webp",
      icon: (
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
    ...
    <section className="section-y">
      <div className="mx-auto max-w-360 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-(--section-padding)">
          {solutions.map((item) => {
            const isFull = item.layout === "full" || item.layout === "full_dark";
            const wrapperClass = isFull ? "lg:col-span-2" : "h-full";

            if (item.layout === "full_dark") {
              return (
                <GsapScrollReveal key={item.id} className={wrapperClass}>
                  <div className="bg-[#0b1120] rounded-3xl lg:rounded-4xl flex flex-col lg:flex-row overflow-hidden text-white shadow-xl h-full">
                    <div className="p-8 lg:p-12 xl:p-16 flex-1 flex flex-col justify-center lg:max-w-[50%]">
                      ...
                      <h3 className="text-[24px] font-semibold uppercase mb-(--heading-space) text-white">{item.title}</h3>
                      <p className="text-[14px] lg:text-[16px] font-semibold text-blue-400 mb-(--inner-space) text-justify">{item.subtitle}</p>
                      <div className="text-[14px] lg:text-[16px] text-slate-300 mb-(--section-padding) leading-relaxed whitespace-pre-line text-justify">
                        {item.description}
                      </div>
                      <Link href="/lien-he" className="group text-[14px] text-blue-400 font-medium hover:text-blue-300 inline-flex items-center gap-1.5 leading-none w-fit transition-colors">
                    ...
                    <div className="w-full lg:flex-1 bg-[#111827] min-h-75 relative flex items-center justify-center p-8">
                      <Image src={item.imagePlaceholder} alt={item.title} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-cover rounded-xl opacity-80 transition-transform duration-500 hover:scale-105" />
                    </div>
                  </div>
                </GsapScrollReveal>
              );
                ...
                <GsapScrollReveal key={item.id} className={wrapperClass}>
                  <div className="bg-white lg:bg-[#f4f6f9] rounded-3xl lg:rounded-4xl flex flex-col lg:flex-row overflow-hidden shadow-[0_2px_20px_rgb(0,0,0,0.04)] lg:shadow-sm border border-slate-100 h-full">
                    <div className="p-6 lg:p-8 flex-1 flex flex-col justify-center lg:max-w-[50%] order-2 lg:order-1">
                      ...
                      <h3 className="md:hidden text-[24px] font-semibold uppercase mb-(--heading-space) text-zinc-900 flex items-baseline gap-(--inner-space) leading-snug">
                        <span className="text-blue-600 text-[1.4rem]">{item.id}</span>
                        {item.title}
                      </h3>
                      
                      {/* Desktop Title */}
                      <h3 className="hidden md:block text-[24px] font-semibold uppercase mb-(--heading-space) text-zinc-900">{item.title}</h3>
            ...
            return (
              <GsapScrollReveal key={item.id} className={wrapperClass}>
                <div className="bg-white rounded-3xl lg:rounded-4xl border border-slate-100 shadow-[0_2px_20px_rgb(0,0,0,0.04)] overflow-hidden flex flex-col xl:flex-row h-full">
                  <div className="p-6 lg:p-8 flex-1 flex flex-col order-2 xl:order-1">
                    ...
                    {/* Desktop Title */}
                    <h3 className="hidden md:block text-[24px] font-semibold uppercase mb-(--heading-space) text-zinc-900 leading-snug">{item.title}</h3>
                    <p className="text-[14px] lg:text-[16px] font-semibold text-blue-600 mb-(--inner-space) text-justify">{item.subtitle}</p>
