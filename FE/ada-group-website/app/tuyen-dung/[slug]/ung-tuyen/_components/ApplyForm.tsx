...
export default function ApplyForm({ job, slug }: { job: Recruitment; slug: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const rawMsg = (error instanceof Error ? error.message : "").replace(/hr@adagroup\.vn/gi, "ada.agency.group@gmail.com");
      const isDuplicate =
      ...
      if (isDuplicate) {
        setAlertInfo({
          ...
          message:
            rawMsg ||
            "Bạn đã ứng tuyển công việc này trước đó rồi. Vui lòng gửi lại CV qua email ada.agency.group@gmail.com nếu cần cập nhật!",
        });
      } else {
        setAlertInfo({
          type: "warning",
              ...
              <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-(--inner-space)">
                ...
                <div>
                  ...
                  <div>
                    ...
                    ></textarea>
                    <p className="text-[12px] text-zinc-400 text-right mt-1">{formData.message.length} / 500</p>
                  </div>
                </div>
              ...
              <table className="w-full text-[14px] text-zinc-600 mb-6 border-collapse">
                <tbody>
                  <tr className="border-b border-slate-50">
                    ...
                    <td className="py-2.5 text-right font-medium text-zinc-800">{job.departmentName || "Khác"}</td>
                  </tr>
                  <tr className="border-b border-slate-50">
