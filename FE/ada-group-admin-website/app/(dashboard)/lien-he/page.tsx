import Link from "next/link";
import { ChevronRight } from "lucide-react";
import ContactsWorkspace, {
  ContactsToolbar,
} from "@/app/(dashboard)/lien-he/_components/ContactsWorkspace";

export default function LienHePage() {
  return (
    <div className="flex flex-1 flex-col gap-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex flex-col gap-2">
          <nav className="flex items-center gap-2 text-sm text-[#434750]">
            <Link href="/" className="hover:text-[#1C1B1B]">
              Trang chủ
            </Link>
            <ChevronRight className="size-3" />
            <span className="font-medium text-[#1C1B1B]">Liên hệ</span>
          </nav>
          <div className="flex flex-col gap-1">
            <h1 className="text-3xl font-semibold text-[#1C1B1B]">
              Liên hệ từ khách hàng
            </h1>
            <p className="text-[#434750]">
              Quản lý và phản hồi các yêu cầu liên hệ từ khách hàng trên
              website.
            </p>
          </div>
        </div>

        <ContactsToolbar />
      </div>

      <ContactsWorkspace />
    </div>
  );
}
