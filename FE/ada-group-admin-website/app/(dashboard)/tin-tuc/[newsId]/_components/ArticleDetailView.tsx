...
import type { ArticleDetail } from "@/app/(dashboard)/tin-tuc/[newsId]/_components/data";

const STATUS_STYLES: Record<ArticleDetail["status"], string> = {
    ...
    <div className="flex flex-1 flex-col gap-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            type="button"
            className="flex h-9 items-center gap-2 rounded-lg bg-[#316EE9] px-4 text-sm font-medium text-white hover:bg-[#316EE9]/90"
          >
          ...
          <Link
            ...
            className="flex h-9 items-center gap-2 rounded-lg bg-[#003274] px-4 text-sm font-medium text-[#7C9DE5] hover:bg-[#003274]/90"
          >
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-3">
        <div className="flex flex-col gap-4 rounded-xl border border-[#C4C6D2] bg-[#FCF9F8] p-8 shadow-xs xl:col-span-2">
          ...
          <div className="relative h-102 w-full overflow-hidden rounded-lg">
            <Image
          ...
          <div className="flex flex-col gap-6">
            {article.content.map((block, index) =>
              block.type === "heading" ? (
                <h3
                  key={index}
                  className="text-[22px] font-semibold text-[#1C1B1B]"
                >
                  {block.text}
                </h3>
              ) : (
                <p
                  key={index}
                  className="text-base leading-7.25 text-[#1C1B1B]"
                >
                  {block.text}
                </p>
              )
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span
                className={`flex items-center gap-1 text-base font-medium ${
                  article.featured ? "text-[#0054CD]" : "text-[#1C1B1B]"
                }`}
              >
                {article.featured && <Star className="size-3.5 fill-[#0054CD]" />}
